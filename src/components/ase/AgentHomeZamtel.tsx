...
export default function AgentHomeZamtel({
    ...
    createPortal(
      ...
      >
        ...
        <div className="relative z-[1] flex h-full min-h-0 w-full flex-col overflow-hidden bg-white shadow-2xl sm:max-w-[min(320px,92vw)]">
          <div
            className="flex shrink-0 items-center justify-between border-b px-4 py-3.5 pt-[max(0.75rem,env(safe-area-inset-top))]"
            style={{ borderColor: FIELD_THEME.border }}
          >
            <span id="field-menu-title" className="text-sm font-semibold tracking-tight" style={{ color: FIELD_THEME.primary }}>
              Menu
            </span>
            <button
              type="button"
          ...
          <nav className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto p-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
            {[
              ...
              <button
                ...
                className="rounded-xl px-3 py-3.5 text-left text-sm font-medium transition-colors hover:bg-slate-100"
                style={{ color: FIELD_THEME.text }}
              >
                {row.label}
              </button>
            ))}
      ...
      >
        ...
        <div className="relative z-[1] flex h-full min-h-0 w-full flex-col overflow-hidden bg-white shadow-2xl sm:max-w-[min(400px,92vw)]">
          <div
            ...
            style={{ borderColor: FIELD_THEME.border }}
          >
            <span id="field-notif-title" className="text-base font-semibold tracking-tight" style={{ color: FIELD_THEME.primary }}>
              Notifications
            </span>
            <button
              type="button"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
            {[
              ...
              <button
                ...
                className="mb-2 w-full rounded-xl border px-3 py-3 text-left transition hover:bg-slate-50"
                style={{ borderColor: FIELD_THEME.border, backgroundColor: FIELD_THEME.surface }}
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="text-sm font-semibold" style={{ color: FIELD_THEME.text }}>
                    {n.title}
                  </span>
                  {n.unread && (
                    <span className="mt-0.5 h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: FIELD_THEME.accent }} />
                  )}
                </div>
                <p className="mt-1 text-[12px] leading-snug" style={{ color: FIELD_THEME.textMuted }}>
                  {n.body}
                </p>
                <p className="mt-2 text-[10px] font-medium" style={{ color: FIELD_THEME.textMuted }}>
                  {n.time}
                </p>
              </button>
            ))}
  ...
  return (
    <div
      className="flex min-h-0 min-w-0 flex-1 flex-col antialiased"
      style={{ fontFamily: "'Inter', 'Manrope', system-ui, sans-serif", backgroundColor: FIELD_THEME.bg, color: FIELD_THEME.text }}
    >
      {menuOverlay}
      {notifOverlay}
      ...
      >
        <div className="pointer-events-none absolute -right-16 -top-24 h-48 w-48 rounded-full bg-white/[0.08]" />
        <div className="relative z-[2] flex items-center justify-between gap-1.5 min-[360px]:gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-2 min-[360px]:gap-3">
            <button
              ...
              className="relative flex h-[50px] w-[50px] shrink-0 items-center justify-center rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[#93C5FD] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1E3A8A]"
              aria-label="Open menu"
            >
              <span className="absolute inset-0 rounded-full border-2 border-white/25" />
              <span className="absolute inset-[10px] flex items-center justify-center rounded-full bg-white shadow-md">
                <Menu size={18} strokeWidth={2} style={{ color: FIELD_THEME.primary }} />
              </span>
            </button>
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            >
              ...
              <span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-red-500 ring-2 ring-white/30" />
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-[3] -mt-1 px-3">
        ...
        >
          <div className="px-4 py-4">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/75">Active territory</p>
            <p className="mt-1 text-[clamp(1.125rem,5.5vw,1.375rem)] font-bold leading-tight tracking-tight text-white">
              {territoryLabel}
            </p>
            <p className="mt-1 text-[13px] text-white/85">{dateStr}</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-white/65">Today&apos;s status</span>
            ...
            <button
              ...
              className="mt-3 text-[13px] font-semibold text-white/95 underline-offset-2 transition hover:text-white"
            >
              View tasks ›
            </button>
          </div>
      ...
      <div className="scrollbar-none mt-3 flex min-h-0 flex-1 flex-col overflow-y-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <section
          className="mx-3 mb-3 overflow-hidden rounded-2xl border bg-white"
          style={{ borderColor: FIELD_THEME.border, boxShadow: field.shadowCard }}
        >
          <div className="grid grid-cols-3 divide-x" style={{ borderColor: FIELD_THEME.border }}>
            <div className="py-4 text-center">
              <div className="text-[26px] font-semibold tabular-nums leading-none" style={{ color: FIELD_THEME.primary }}>
                {routeStats.visited}
              </div>
              <div className="mt-1.5 text-[11px] font-medium" style={{ color: FIELD_THEME.textMuted }}>
                Visited
              </div>
            </div>
            <div className="py-4 text-center">
            ...
            <div className="py-4 text-center">
              <div
                ...
                style={{ color: routeStats.remaining > 0 ? REMAINING_WARN : FIELD_THEME.primary }}
              >
          ...
          <p className="border-t px-3 py-2 text-center text-[10px] font-medium" style={{ borderColor: FIELD_THEME.border, color: FIELD_THEME.textMuted }}>
            {routeStats.hint}
          </p>

          <div className="relative z-20 mx-3 mb-3 flex gap-1 rounded-xl p-1 touch-manipulation" style={{ backgroundColor: '#E5E7EB' }}>
            {(['daily', 'weekly', 'monthly'] as const).map((key) => (
              <button
                ...
                className={`relative z-30 flex-1 cursor-pointer rounded-lg py-2.5 text-[12px] font-semibold transition-all select-none active:scale-[0.98] ${
                  periodTab === key ? 'bg-white shadow-sm ring-1 ring-[#D1D5DB]' : ''
                }`}
                style={periodTab === key ? { color: FIELD_THEME.primary } : { color: FIELD_THEME.textMuted }}
              >
          ...
          <div className="px-3 pb-4">
            <div
              ...
              style={{ background: field.kpiGradient }}
            >
              ...
              <div className="text-[12px] font-medium text-white/90">Outlet Visited</div>
              <div className="mt-1 text-[26px] font-semibold tabular-nums tracking-tight text-white">
                {vp.visitA}
                <span className="text-[15px] font-normal text-white/60"> / {vp.visitT}</span>
              </div>
              <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/25">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${pct(vp.visitA, vp.visitT)}%`, backgroundColor: FIELD_THEME.focus }}
                />
              </div>
            </div>
          </div>
        ...
        <section
          className="mx-3 mb-3 overflow-hidden rounded-2xl border bg-white p-4"
          style={{ borderColor: FIELD_THEME.border, boxShadow: field.shadowCard }}
        >
          <h2 className="text-[15px] font-semibold tracking-tight" style={{ color: FIELD_THEME.text }}>
            Shortcuts
          </h2>
          <p className="mt-1 text-[11px]" style={{ color: FIELD_THEME.textMuted }}>
            Other capture steps happen during the visit flow.
          </p>
          <button
            type="button"
            onClick={() => setActiveView('route')}
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-[14px] font-semibold text-white shadow-md transition active:scale-[0.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#93C5FD] focus-visible:ring-offset-2"
            style={{ backgroundColor: FIELD_THEME.primary, boxShadow: field.shadowElevated }}
          >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2.5">
            <button
              type="button"
              onClick={() => setActiveView('route')}
              className="flex items-center gap-3 rounded-xl border bg-slate-50/80 px-3 py-3 text-left transition hover:bg-slate-100 active:scale-[0.99]"
              style={{ borderColor: FIELD_THEME.border }}
            >
              <div
              ...
              <div>
                <div className="text-[13px] font-semibold" style={{ color: FIELD_THEME.text }}>
                  Route map
                </div>
                <div className="text-[11px]" style={{ color: FIELD_THEME.textMuted }}>
        ...
        >
          <div className="flex items-center justify-between px-4 py-3.5">
            <div className="text-[15px] font-semibold" style={{ color: FIELD_THEME.text }}>
              Route Queue
            </div>
            <button
              type="button"
          ...
          <div
            className="relative mx-3 mb-2 flex h-[76px] items-center justify-center overflow-hidden rounded-xl border bg-gradient-to-b from-slate-50 to-slate-100/80"
            style={{ borderColor: FIELD_THEME.border }}
          >
            <div className="absolute left-[8%] right-[8%] top-1/2 h-0.5 -translate-y-1/2 bg-slate-300/60" />
              ...
              {routePreview.map((_, i) => (
                <span
                  ...
                  className={`rounded-full shadow-sm transition-all ${
                    i === 0 ? 'h-3.5 w-3.5 bg-[#0B5ED7] ring-2 ring-[#93C5FD]/50' : 'h-2.5 w-2.5 bg-slate-300'
                  }`}
                />
              ))}
            </div>
          ...
          {routePreview.map((o, idx) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-center gap-3 border-t px-4 py-3.5 text-left transition hover:bg-slate-50 active:bg-slate-100/80"
                style={{ borderColor: FIELD_THEME.border }}
              >
                <span
                  className="h-2.5 w-2.5 shrink-0 rounded-full"
                  style={{ backgroundColor: idx === 0 ? FIELD_THEME.primary : '#CBD5E1' }}
                />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[14px] font-semibold" style={{ color: FIELD_THEME.text }}>
                    {o.name}
                  </div>
                  <span
                    className={`mt-1 inline-block rounded-md px-2 py-0.5 text-[10px] font-semibold ${
                      badge === 'mm' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-50 text-[#0B5ED7]'
                    }`}
                  >
                    {o.category}
                  </span>
                </div>
                <span className="shrink-0 text-[12px] font-medium" style={{ color: FIELD_THEME.textMuted }}>
                  {o.distance}
                </span>
        ...
        <section
          className="mx-3 mb-[max(1.25rem,env(safe-area-inset-bottom))] overflow-hidden rounded-2xl border bg-white"
          style={{ borderColor: FIELD_THEME.border, boxShadow: field.shadowCard }}
        >
          <div className="px-4 py-3.5">
            <div className="text-[15px] font-bold tracking-tight" style={{ color: FIELD_THEME.primary }}>
              Quick Links
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 px-4 pb-5 sm:grid-cols-3">
            {[
              ...
              <button
                ...
                type="button"
                onClick={q.onClick}
                className="group flex flex-col items-center gap-2 rounded-xl py-1 transition active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#93C5FD]"
              >
                <div
                  className="flex h-14 w-14 items-center justify-center rounded-2xl border bg-slate-50 transition group-hover:bg-slate-100"
                  style={{ borderColor: FIELD_THEME.border }}
                >
                  <q.icon size={24} strokeWidth={1.75} style={{ color: FIELD_THEME.primary }} />
                </div>
                <span className="text-center text-[11px] font-medium leading-snug" style={{ color: FIELD_THEME.text }}>
                  {q.label}
                </span>
              </button>
            ))}
